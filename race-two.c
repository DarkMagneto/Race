...
void deplacement_two(struct player *self,const struct checkpoint *obj, const struct grid *grille){
    if(self->px<obj->x){
        switch(self->vx){
            case 1 :{
                self->px++;
...
int main(){
    ...
    for(;;){
        deplacement_two(&two,&objective,&grille);
